...
const getUserByIdSocket = async ({ user_id }) => {
	return new Promise((resolve, reject) => {
        ...
        socket.on("ice-candidate", ({ to, from, candidate }) => {
            const targetSocketId = users[to.username];
            if (targetSocketId) {
                socket.to(targetSocketId).emit("ice-candidate", {
                    from,
                    candidate,
                });
                console.log(`❄️ ICE candidate sent from ${from.username} to ${to.username}`);
            } else {
                console.warn(`⚠️ User ${to.username} not found for ICE`);
            }
        });
